  ...
  try {
    ...
    if (book.userId.toString() !== req.user.id.toString()) {
      ...
      return res.status(403).json({
        error: "Forbidden: You cannot update this book cover!",
      });
    }

    if (book.coverImage) {
      const oldImagePath = path.join(__dirname, "../uploads", path.basename(book.coverImage));
...
async function deleteBook(req, res) {
  try {
    ...
    if (book.coverImage) {
      const imagePath = path.join(__dirname, "../uploads", path.basename(book.coverImage));
