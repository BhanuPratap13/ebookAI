import { BookOpen, Download, Lightbulb, Library } from "lucide-react";

export const FEATURES = [
  {
    title: "AI Writing Assistant",
    description:
      "Generate ideas, outlines, and chapter drafts with smart AI support.",
    icon: Lightbulb,
    bgGradientColors: "from-violet-500 to-purple-600",
    shadowColor: "shadow-violet-500/50",
  },
  {
    title: "Live Reader Preview",
    description:
      "View your book in a distraction-free reader mode with adjustable fonts.",
    icon: BookOpen,
    bgGradientColors: "from-blue-500 to-cyan-600",
    shadowColor: "shadow-blue-500/50",
  },
  {
    title: "Instant Export",
    description:
      "Download your book in DOCX or PDF with a single click—no setup required.",
    icon: Download,
    bgGradientColors: "from-emerald-500 to-teal-600",
    shadowColor: "shadow-emerald-500/50",
  },
  {
    title: "Project Dashboard",
    description:
      "Manage drafts, track progress, and organize all your books in one place.",
    icon: Library,
    bgGradientColors: "from-pink-500 to-rose-600",
    shadowColor: "shadow-pink-500/50",
  },
];

export const TESTIMONIALS = [
  {
    username: "Aarav Sharma",
    designation: "Aspiring Novelist",
    quote:
      "I came in with an idea and left with a book. eBookAI made the entire writing journey feel simple, focused, and genuinely exciting.",
    avatarSrc: "/images/images.jpeg",
    rating: 5,
  },
  {
    username: "Sarah Lee",
    designation: "Self-Published Author",
    quote:
      "eBookAI gives me the momentum I need to actually finish what I start. It's become an essential part of my writing workflow.",
    avatarSrc: "/images/images (1).jpeg",
    rating: 5,
  },
  {
    username: "Emily Carter",
    designation: "Independent Publisher",
    quote:
      "The biggest difference is speed without sacrificing quality. eBookAI helps me turn rough concepts into polished, publication-ready books.",
    avatarSrc: "/images/images (2).jpeg",
    rating: 5,
  },
];

export const WRITING_STYLES = [
  "Informative",
  "Formal",
  "Conversational",
  "Professional",
  "Academic",
  "Narrative",
  "Creative",
  "Descriptive",
  "Technical",
  "Instructional",
  "Inspirational",
  "Motivational",
  "Humorous",
  "Sarcastic",
  "Witty",
  "Poetic",
  "Storytelling",
  "Journalistic",
  "Analytical",
  "Critical",
  "Persuasive",
  "Emotional",
  "Dramatic",
  "Mystical",
  "Reflective",
  "Philosophical",
  "Minimalist",
  "Casual",
  "Friendly",
  "Bold",
  "Direct",
  "Objective",
  "Subjective",
  "Uplifting",
  "Melancholic",
  "Dark",
  "Satirical",
  "Whimsical",
  "Playful",
  "Epic",
  "Fantasy",
  "Sci-Fi",
  "Technical Coding",
  "Business",
  "Marketing",
  "Product Copy",
  "Academic Research",
  "Whitepaper",
  "Case Study",
  "SEO Optimized",
];